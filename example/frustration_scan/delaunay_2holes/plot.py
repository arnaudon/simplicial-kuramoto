...
if __name__ == "__main__":
    ...
    order_1 = compute_simplicial_order_parameter(res, harm_subspace, subset=list(subset))

    subset = np.zeros(Gsc.n_edges, dtype=bool)
    subset[int(2*Gsc.n_edges/3):] = True
    order_2 = compute_simplicial_order_parameter(res, harm_subspace, subset=list(subset))

    order = compute_simplicial_order_parameter(res, harm_subspace)
    plt.figure()
